...
using RehberlikSistemi.Web.Services;
using Xunit;
...
namespace RehberlikSistemi.Web.Tests.Controllers
{
    public class TeacherControllerTests : IDisposable
    {
        ...
        private readonly Mock<IPlanGenerationService> _mockPlanService;
        ...
        public TeacherControllerTests()
        {
            ...
            _mockPlanService = new Mock<IPlanGenerationService>();
            _mockDataProtectionProvider = new Mock<IDataProtectionProvider>();
            _mockProtector = new Mock<IDataProtector>();

            _mockDataProtectionProvider.Setup(s => s.CreateProtector(It.IsAny<string>())).Returns(_mockProtector.Object);

            _teacherUser = new ApplicationUser { Id = "teacher1", UserName = "teacher@test.com", FirstName = "Teacher", LastName = "One" };

            var claims = new List<Claim> { new Claim(ClaimTypes.NameIdentifier, _teacherUser.Id) };
            var identity = new ClaimsIdentity(claims, "TestAuthType");
            var claimsPrincipal = new ClaimsPrincipal(identity);

            _controller = new TeacherController(_context, _mockUserManager.Object, _mockPlanService.Object, _mockDataProtectionProvider.Object)
            {
        ...
        {
            // Arrange - we already have teacher1 and student1 in ctor. Let's add more for dashboard testing.
            var subject1 = await _context.Subjects.FindAsync(1);
            
            var studentUser1 = new ApplicationUser { Id = "student_u1", FirstName = "John", LastName = "Doe" };
            _student.User = studentUser1; // Link student1 from ctor to a user

            var studentUser2 = new ApplicationUser { Id = "student_u2", FirstName = "Jane", LastName = "Smith" };
            var studentProfile2 = new StudentProfile { Id = 2, UserId = "student_u2", User = studentUser2, TeacherId = _teacherUser.Id, GradeLevel = "11" };

            var studentUser3 = new ApplicationUser { Id = "student_u3", FirstName = "Other", LastName = "Student" };
            var studentProfile3 = new StudentProfile { Id = 3, UserId = "student_u3", User = studentUser3, TeacherId = "otherTeacherId", GradeLevel = "12" };

            _context.StudentProfiles.AddRange(studentProfile2, studentProfile3);
        ...
        public async Task CreateStudyTask_ValidModel_ReturnsRedirectToStudentDetail()
        {
            ...
            Assert.Equal(model.ScheduledDate.Date, createdTask.ScheduledDate.Date);
        ...
        public async Task AssignStudent_UserNotFound_ReturnsNotFound()
        {
            ...
            // Act
            var result = await _controller.AssignStudent(1);
        ...
        {
            // Act
            var result = await _controller.AssignStudent(999);
        ...
        {
            // Arrange
            var student = new StudentProfile { Id = 10, UserId = "student_no_teacher", TeacherId = null };
            _context.StudentProfiles.Add(student);
            await _context.SaveChangesAsync();

            // Act
            var result = await _controller.AssignStudent(10);

            // Assert
            var redirectResult = Assert.IsType<RedirectToActionResult>(result);
            Assert.Equal("Students", redirectResult.ActionName);

            var updatedStudent = await _context.StudentProfiles.FindAsync(10);
            Assert.NotNull(updatedStudent);
            Assert.Equal(_teacherUser.Id, updatedStudent.TeacherId);
        }
        ...
        {
            // Arrange
            var student = new StudentProfile { Id = 10, UserId = "student_with_teacher", TeacherId = "otherTeacherId" };
            _context.StudentProfiles.Add(student);
            await _context.SaveChangesAsync();
            ...
            var updatedStudent = await _context.StudentProfiles.FindAsync(10);
            Assert.NotNull(updatedStudent);
            Assert.Equal("otherTeacherId", updatedStudent.TeacherId);
        }
        ...
        {
            // Act
            var result = await _controller.PlanGenerator(999);
        ...
        {
            // Arrange
            var studentUser = new ApplicationUser { Id = "s_no_avail", FirstName = "John", LastName = "Doe" };
            ...
            // Act
            var result = await _controller.PlanGenerator(10) as ViewResult;
        ...
        {
            // Arrange
            var studentUser = new ApplicationUser { Id = "s_priority", FirstName = "John", LastName = "Doe" };
            var subject2 = new Subject { Id = 2, Name = "Physics" };
            var subject3 = new Subject { Id = 3, Name = "Chemistry" };
            _context.Subjects.AddRange(subject2, subject3);

            // Use the next Monday (first day of generated plan)
            var nextMonday = DateTime.Now.Date.GetStartOfWeek().AddDays(7);
            var dayOfWeek = nextMonday.DayOfWeek; // Monday
            ...
            var studentProfile = new StudentProfile
            {
                ...
                Availabilities = new List<Availability>
                {
                    new Availability { DayOfWeek = dayOfWeek, IsAvailable = true, StartTime = new TimeSpan(10, 0, 0), EndTime = new TimeSpan(15, 0, 0) }
                },
                Exams = new List<Exam>
                {
                    new Exam { SubjectId = 1, ExamDate = DateTime.Now.AddDays(3), ImportanceLevel = 5 },
                    new Exam { SubjectId = 2, ExamDate = DateTime.Now.AddDays(-10), Score = 40 }
                }
            };
            ...
            // Subject 1 has highest priority
        ...
        {
            // Arrange
            var studentUser = new ApplicationUser { Id = "s_exam_inc", FirstName = "John", LastName = "Doe" };
            var subject2 = new Subject { Id = 2, Name = "Physics" };
            ...
            var studentProfile = new StudentProfile
            {
                ...
                Availabilities = new List<Availability>
                {
                    new Availability { DayOfWeek = dayOfWeek, IsAvailable = true, StartTime = new TimeSpan(10, 0, 0), EndTime = new TimeSpan(13, 0, 0) }
                },
                Exams = new List<Exam>
                {
                    new Exam { SubjectId = 2, ExamDate = DateTime.Now.AddDays(-10), Score = 60 }
                }
            };
        ...
        [Fact]
        public async Task PlanGenerator_ExistingTodayTasks_StillGeneratesPlan()
        {
            // Arrange: Add a task for today — this should NOT block next week's plan generation
            var studentUser = new ApplicationUser { Id = "s_today", FirstName = "Today", LastName = "Student" };
            ...
            var studentProfile = new StudentProfile
            {
                ...
                Availabilities = new List<Availability>
                {
                    new Availability { DayOfWeek = DayOfWeek.Monday, IsAvailable = true, StartTime = new TimeSpan(9, 0, 0), EndTime = new TimeSpan(12, 0, 0) }
                },
                Exams = new List<Exam>()
            };
            _context.StudentProfiles.Add(studentProfile);

            // Add an existing task for today
            ...
            // Assert: Plan should be generated successfully, today's task should not block it
            Assert.NotNull(result);
            var model = Assert.IsType<PlanGeneratorViewModel>(result.Model);
            Assert.True(model.IsSuccessful);
            Assert.True(model.ProposedTasks.Count > 0);
            // All generated tasks should be in the next week (Monday onward)
            Assert.All(model.ProposedTasks, t => Assert.True(t.ScheduledDate >= nextMonday));
        }
    }
}
